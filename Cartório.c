...
int deletar() //função de deletar usuários
{
	...
	printf("Digite o cpf do usuï¿½rio que vocï¿½ quer deletar: "); //definindo a variável cpf
	scanf("%s",cpf);
	...
	if(file == NULL){ //resposta caso o usuário não exista
		printf("O Usuï¿½rio nï¿½o foi encontrado no sistema !\n "); 
		fclose(file);
	}
	
	else{ //deletando o arquivo do usuário
		printf("o usuï¿½rio foi deletado do sistema!\n");
		fclose(file);
		remove(cpf);
	}
